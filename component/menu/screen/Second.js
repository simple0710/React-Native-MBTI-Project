...
import { TouchableOpacity, View, FlatList, Dimensions, BackHandler } from "react-native";
...
const Second = ({ route } ) => {
  ...
  const mbtiMatch = 
  {
    labels : {"INFP" : 0, "ENFP" : 1, "INFJ" : 2, "ENFJ" : 3,
  ...
  const renderMbtiMath = ({item}) => {
    ...
    if (clickName != item.name) {
      return (
        ...
        backgroundColor:
          mbtiMatch.backcolor[mbtiMatch.graph[mbtiMatch.labels[nowMbti]][mbtiMatch.labels[item.labels[0]]]]
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <>
      <TouchableOpacity style={{width: Dimensions.get("window").width / 3, minHeight: 75,justifyContent: "center", alignItems: "center",borderColor: "black", borderWidth: 1, borderStyle: "solid"}} 
      onPress={hendlePress}
      >
        <Text>{item.name}</Text>
        <View>
  ...
  return (
    <>
      ...
      {clickFlag &&
      <>
      <View style={{position: "absolute", top: 0, left: 0, width: Dimensions.get("window").width / 3, height: Dimensions.get("window").height, backgroundColor: "blue"}}>
      {/* 왼쪽 선택 가능 스크롤 */}
      <FlatList
          data={mbtiData}
          renderItem={renderItem}
      ...
      {/* 오른쪽 궁합판 */}
        ...
        <View style={{position: "absolute", top: 0, right: 0, width: Dimensions.get("window").width / 3 * 2, height: Dimensions.get("window").height, backgroundColor: "white"}}>
          <Text>{clickName}</Text>
          <FlatList
            data={mbtiData}
